...
type ContentReportMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function ContentReportsMultiDeleteDialog<TData>({
  open,
  onOpenChange,
  table,
}: ContentReportMultiDeleteDialogProps<TData>) {
    ...
    try {
      await Promise.all(
        selectedRows.map((row) =>
          apiClient.delete(
            `/admin/content-reports/${(row.original as ContentReport).id}`
          )
        )
      )
  ...
  return (
    <ConfirmDialog
      ...
      form='content-reports-multi-delete-form'
      ...
      desc={
        <form
          id='content-reports-multi-delete-form'
          onSubmit={(e) => {
            e.preventDefault()
            handleDelete()
          }}
