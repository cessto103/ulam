    ...
    try {
      await Promise.all(
        selectedRows.map((row) =>
          apiClient.delete(`/admin/comments/${(row.original as PostComment).id}`)
        )
      )
      ...
      onOpenChange(false)
      toast.success(
        `Deleted ${selectedRows.length} ${selectedRows.length > 1 ? 'comments' : 'comment'}`
      )
    } catch {
    ...
    <ConfirmDialog
      ...
      title={
        <span className='text-destructive'>
          ...
          {selectedRows.length > 1 ? 'comments' : 'comment'}
        </span>
      }
      desc={
        <form
